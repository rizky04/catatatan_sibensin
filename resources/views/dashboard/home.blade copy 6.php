    ...
    <section class="p-6 space-y-6 pb-24">
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm space-y-4"
             x-data="{
                ...
                async scanReceipt(event) {
                    ...
                    try {
                        ...
                        // Ganti blok fetch di dalam script kamu dengan ini:
                        const response = await fetch('{{ route('ai.scan') }}', {
                            method: 'POST',
                            headers: {
                                // Ini kuncinya agar Laravel tidak menolak (419 Error)
                        ...
                        if(data.fuel_type) {
                            ...
                            else if(ft.includes('pertalite')) this.fuel_type = 'Pertalite';
                            else if(ft.includes('dexlite')) this.fuel_type = 'Dexlite';
                            else if(ft.includes('dex')) this.fuel_type = 'Pertamina Dex';
                        }

                        alert('✨ Struk terbaca! Periksa kembali data dan isi Odometer.');
                    } catch (error) {
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                @if($vehicle)
                    <label class="bg-purple-100 text-purple-600 px-3 py-2 rounded-xl font-black text-[10px] uppercase tracking-wider flex items-center gap-2 cursor-pointer hover:bg-purple-200 transition-all shadow-sm border border-purple-200 active:scale-95">
            ...
            @if($vehicle)
                <form action="{{ route('fuel.store') }}" method="POST" class="space-y-4 relative">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-gray-50 p-4 rounded-2xl border transition-all duration-500" :class="isAiGenerated ? 'border-purple-300 bg-purple-50/50' : 'border-gray-100'">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div class="bg-gray-50 p-3 rounded-2xl border transition-all duration-500" :class="isAiGenerated ? 'border-purple-300 bg-purple-50/50' : 'border-gray-100'">
